...
public class Laptop {
    ...
    public Laptop(String name, boolean internet, int weight, int memory, String system, int battery, boolean isWorking){
        this.name = name;
    ...
    void turnON() { // Нельзя включить, если включен, а также если села батарея
        if (isWorking == false) {
            ...
            isWorking = true;
            System.out.println("Ноутбук включается, пожалуйста подождите.");
    ...
    void work() { // Нельзя работать если выключен. Также переходят в состояние выключен, когда села батарея.
        ...
        {
            battery -= 10;
            System.out.println("Вы в ноутбуке какое-то время, батарея садится.");
            ...
            if (battery <= 50 && battery >= 10) {
                ...
                isWorking = false;
                System.out.println("Ноутбук сел. " + battery + " заряда батареи");
            }
        } else {
            System.out.println("Сначало включите/зарядите ноутбук. Текущая батарея: " + battery);
        }
    ...
    void turnOFF() { // Нельзя выключить если выключен.
        if (isWorking == true) {
            ...
            if (battery == 0) {
                ...
                System.out.println("Ноутбук выключается, подождите его выключения, прежде чем закрыть.");
