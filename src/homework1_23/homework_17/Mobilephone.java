package homework1_23.homework_17;
...
public class Mobilephone {
    ...
    public Mobilephone(String name, boolean internet, double weight, double memory, String system, int battery, boolean isWorking){
        this.name = name;
    ...
    void turnON() { // Нельзя включить, если включен, а также если села батарея
        if (isWorking == false) {
            ...
            isWorking = true;
            System.out.println("Телефон включается, пожалуйста подождите.");
    ...
    void work() { // Нельзя работать если выключен. Также переходят в состояние выключен, когда села батарея.
        ...
        {
            battery -= 10;
            System.out.println("Вы в телефоне какое-то время, батарея садится.");
            ...
            if (battery <= 50 && battery >= 10) {
                ...
                isWorking = false;
                System.out.println("Телефон сел. " + battery + " заряда батареи");
            }
        } else {
            System.out.println("Сначало включите/зарядите телефон. Текущая батарея: " + battery);
        }
    ...
    void turnOFF() { // Нельзя выключить если выключен.
        if (isWorking == true) {
            ...
            if (battery == 0) {
                ...
                System.out.println("Телефон выключается, подождите его выключения, прежде чем закрыть.");
